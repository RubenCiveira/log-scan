...
def run(agent: Agent, prompt: str):
    ...
    if not api_key:
        sys.stderr.write( json.dumps({
            "system": "Error: falta OPENAI_API_KEY en el entorno"
...
def enable_global_callback_injection(callbacks, streaming=False, tags=None, metadata=None):
    orig_init = ChatOpenAI.__init__

    def _patched_init(self, *args, **kwargs):
        # no dupliques callbacks si ya vinieron en kwargs
        if "callbacks" not in kwargs or not kwargs["callbacks"]:
            kwargs["callbacks"] = callbacks
        # respeta explícito del llamador; si no, aplica streaming por defecto global
        if "streaming" not in kwargs:
            kwargs["streaming"] = streaming
        obj = orig_init(self, *args, **kwargs)
        try:
            self = self.with_config(callbacks=callbacks, tags=tags or [], metadata=metadata or {})
        except Exception:
            pass
        return obj
